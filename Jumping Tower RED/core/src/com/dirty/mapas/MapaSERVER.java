...
public class MapaSERVER {
	...
	float ppm=ConfiguracionesSERVER.PPM.getValor();
		...
		renderizarMapa  =  new OrthogonalTiledMapRenderer(mapa,1/UtilesSERVER.PPM);
	...
	//crea hitbox bandera
	public void realizarHitboxBandera(BodyDef bdef, Body cuerpo, FixtureDef fdef, PolygonShape shape, World mundo) {
		for(MapObject objetoMapa: mapa.getLayers().get(6).getObjects().getByType(RectangleMapObject.class)) {
			rect = ((RectangleMapObject) objetoMapa).getRectangle();
			bdef.type=BodyDef.BodyType.StaticBody;
			bdef.position.set(rect.getX()+rect.getWidth()/2,rect.getY()+rect.getHeight()/2);
